...
<div class="alert alert-danger mb-4" style="background: #f8d7da; border: 1px solid #f5c6cb; color: #721c24; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
    <h5><i class="fa-solid fa-triangle-exclamation"></i> Peringatan Stok Rendah!</h5>
    @foreach($barangStokRendah as $barang)
        <p style="margin: 5px 0;"><strong>{{ $barang->nama_barang }}</strong> ({{ $barang->merk_barang }}): 
        Sisa <span style="color: #dc3545; font-weight: bold;">{{ $barang->stok }}</span> | 
        Min: {{ $barang->stok_minimum ?? 10 }}</p>
    @endforeach
</div>
@endif
